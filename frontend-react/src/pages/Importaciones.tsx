...
export default function Importaciones() {
  ...
  const detectarTienda = (productUrl: string) => {
  ...
  const handleModifyItem = (id: number, field: keyof ImportItem, value: string) => {
  ...
  const triggerCheckout = (e: React.MouseEvent<HTMLButtonElement>) => {
    e.preventDefault();
    if (items.length === 0) return;
  ...
  const processOrder = async (nombre: string, telefono: string) => {
    setSubmitting(true);
  ...
  const sendWhatsAppMessage = (nombre: string, telefono: string) => {
    ...
    items.forEach((item, i) => {
      ...
      messageLines.push(`🔗 *Enlace:* ${item.url}`);
    ...
    messageLines.push("");
    messageLines.push(`💵 *Abono inicial (50% para iniciar):* $${totals.inicialUsd.toFixed(2)} (*${totals.inicialBs.toFixed(2)} Bs*)`);
    messageLines.push(`💵 *Monto al retirar (50% restante al retirar):* $${totals.inicialUsd.toFixed(2)} (*${totals.inicialBs.toFixed(2)} Bs*)`);
    messageLines.push("");
      ...
      <div className="min-h-[70vh] flex items-center justify-center p-6 bg-gray-50/50 dark:bg-gray-950/20 animate-fade-in">
        <div className="max-w-md w-full bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-3xl p-8 text-center shadow-xl shadow-primary-100/50 dark:shadow-black/25 hover:shadow-primary-200/50 dark:hover:border-primary-900/40 transition-all duration-200 animate-scale-in">
          ...
          <div className="bg-primary-50 dark:bg-primary-950/30 border border-primary-100 dark:border-primary-900/50 py-4 px-6 rounded-2xl mb-8">
            <span className="text-3xl font-mono font-black tracking-widest text-primary-600 dark:text-primary-400">{successCode}</span>
          </div>
  ...
  return (
    <div className="min-h-screen bg-gray-50/50 dark:bg-gray-950 text-gray-900 dark:text-gray-100 font-sans selection:bg-primary-100 dark:selection:bg-primary-950/30 selection:text-primary-900 dark:selection:text-primary-400 transition-colors duration-200">
      ...
      <div className="relative max-w-7xl mx-auto py-6 px-4 sm:px-6 lg:px-8 animate-fade-in">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between mb-8 space-y-4 md:space-y-0">
          <div className="text-center md:text-left">
            ...
            <p className="text-gray-500 dark:text-gray-400 text-sm mt-2">
              Cótiza el costo total de traer tus productos de Amazon, AliExpress, eBay y más tiendas hasta tu puerta en Venezuela.
            </p>
          </div>
          {tasaVes && (
        ...
        <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-3xl p-6 shadow-sm mb-8 transition-colors duration-200 animate-slide-up delay-75">
          <h2 className="text-lg font-bold text-gray-900 dark:text-white mb-4">Añadir artículo a cotización</h2>
          <form onSubmit={handleAddItem} className="grid grid-cols-1 md:grid-cols-12 gap-4 items-end" noValidate>

            <div className="md:col-span-6">
              <label htmlFor="urlProducto" className="block text-xs font-bold uppercase tracking-wider text-gray-500 dark:text-gray-400 mb-2">Enlace del producto</label>
              <input
                id="urlProducto"
                type="url"
                required
                ...
                className="w-full bg-gray-50 dark:bg-gray-950 border border-gray-200 dark:border-gray-800 rounded-xl px-4 py-3 text-gray-900 dark:text-white focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary-500 focus:border-primary-500 dark:focus:border-primary-500 focus:bg-white dark:focus:bg-gray-900 transition-all duration-200 text-sm"
                placeholder="https://www.amazon.com/dp/..."
              />
            </div>

            <div className="md:col-span-2">
              <label htmlFor="precioProducto" className="block text-xs font-bold uppercase tracking-wider text-gray-500 dark:text-gray-400 mb-2">Precio Tienda ($)</label>
              <input
                id="precioProducto"
                type="number"
                step="0.01"
            ...
            <div className="md:col-span-2">
              <label htmlFor="pesoProducto" className="block text-xs font-bold uppercase tracking-wider text-gray-500 dark:text-gray-400 mb-2">Peso Est. (Lbs)</label>
              <input
                id="pesoProducto"
                type="number"
                step="1"
            ...
            <div className="md:col-span-2">
              <button
                type="submit"
                className="w-full bg-primary-600 hover:bg-primary-500 text-white font-bold py-3.5 px-4 rounded-xl transition-all duration-200 text-sm flex items-center justify-center gap-2 shadow-md shadow-primary-600/20 hover:shadow-primary-500/50 hover:-translate-y-0.5 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-offset-2 focus-visible:ring-primary-500 dark:focus-visible:ring-offset-gray-900 active:scale-95"
              >
        ...
        <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-3xl overflow-hidden shadow-sm mb-8 transition-colors duration-200 animate-slide-up delay-150">
          <div className="flex justify-between items-center px-6 py-4 border-b border-gray-100 dark:border-gray-800">
            ...
            {items.length > 0 && (
              <button
                ...
                className="text-sm font-bold text-primary-600 hover:text-primary-700 transition-colors flex items-center gap-1.5 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary-500 rounded-md px-2 py-1"
              >
                ...
                {items.length === 0 ? (
                  <tr>
                    <td colSpan={6} className="px-6 py-16 text-center text-gray-500 dark:text-gray-400">
                      <i className="bi bi-box-seam text-gray-300 dark:text-gray-600 text-5xl mb-4 block" aria-hidden="true"></i>
                  ...
                  items.map(item => {
                    ...
                    return (
                      <tr key={item.id} className="border-b border-gray-100 dark:border-gray-800/60 hover:bg-gray-50/40 dark:hover:bg-gray-950/20 transition-colors duration-200">
                        <td className="px-6 py-4">
                          <span className={`inline-block px-2.5 py-1 rounded-md text-xs font-bold uppercase tracking-wider transition-colors duration-200
                        ...
                        <td className="px-6 py-4 max-w-[200px] truncate">
                          <a href={item.url} target="_blank" rel="noopener noreferrer" aria-label={`Ver artículo en ${item.tienda}`} className="text-sm font-semibold text-primary-600 hover:text-primary-700 hover:underline flex items-center gap-1 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary-500 rounded-sm">
                            Ver Artículo
                            <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true"><path strokeLinecap="round" strokeLinejoin="round" d="M10 6H6a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2v-4M14 4h6m0 0v6m0-6L10 14" /></svg>
                          </a>
                        </td>
                        <td className="px-6 py-4 text-center">
                          <input
                            type="number"
                            step="0.01"
                            aria-label={`Precio del artículo ${item.tienda}`}
                            value={item.precio}
                            onChange={e => handleModifyItem(item.id, 'precio', e.target.value)}
                            className="w-20 bg-gray-50 dark:bg-gray-950 border border-gray-200 dark:border-gray-800 rounded-lg py-1.5 px-2 text-center text-sm font-bold text-gray-900 dark:text-white focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary-500 transition-colors"
                          />
                        </td>
                        <td className="px-6 py-4 text-center">
                          <input
                            type="number"
                            step="1"
                            aria-label={`Peso del artículo ${item.tienda}`}
                            value={item.peso}
                            onChange={e => handleModifyItem(item.id, 'peso', e.target.value)}
                            className="w-16 bg-gray-50 dark:bg-gray-950 border border-gray-200 dark:border-gray-800 rounded-lg py-1.5 px-2 text-center text-sm font-bold text-gray-900 dark:text-white focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary-500 transition-colors"
                          />
                        </td>
                        ...
                        <td className="px-6 py-4 text-center">
                          <button
                            ...
                            className="text-primary-600 hover:text-primary-700 p-1.5 bg-primary-50 dark:bg-primary-950/20 border border-primary-100 dark:border-primary-900/50 rounded-lg transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary-500"
                            title="Eliminar artículo"
                            aria-label={`Eliminar artículo ${item.tienda}`}
                          >
          ...
          <div className="block md:hidden divide-y divide-gray-100 dark:divide-gray-800/60">
            {items.length === 0 ? (
              ...
              items.map(item => {
                ...
                return (
                  <div key={item.id} className="p-5 space-y-4 hover:bg-gray-50/20 dark:hover:bg-gray-950/10 transition-colors duration-200">
                    ...
                    `}>
                      ...
                      <button
                        type="button"
                        onClick={() => handleRemoveItem(item.id)}
                        className="text-primary-600 hover:text-primary-700 p-2 bg-primary-50 dark:bg-primary-950/20 border border-primary-100 dark:border-primary-900/50 rounded-lg transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary-500"
                        title="Eliminar artículo"
                        aria-label={`Eliminar artículo ${item.tienda}`}
                      >
                        <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" /></svg>
                      </button>
                    </div>

                    <div className="flex flex-col gap-1.5">
                      <span className="text-xs font-bold uppercase tracking-wider text-gray-400 dark:text-gray-500">Enlace</span>
                      <a href={item.url} target="_blank" rel="noopener noreferrer" className="text-sm font-semibold text-primary-600 hover:text-primary-700 dark:text-primary-400 dark:hover:text-primary-500 hover:underline flex items-center gap-1 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary-500 rounded-sm w-fit">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        ...
                        <input
                          ...
                          value={item.precio}
                          onChange={e => handleModifyItem(item.id, 'precio', e.target.value)}
                          className="w-full bg-gray-50 dark:bg-gray-950 border border-gray-200 dark:border-gray-800 rounded-xl py-2 px-3 text-sm font-bold text-gray-900 dark:text-white focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary-500 transition-colors"
                        />
                      </div>
                      <div>
        ...
        {/* Políticas */}
        <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-3xl p-6 shadow-sm hover:shadow-[0_15px_40px_-10px_rgba(225,29,72,0.15)] dark:hover:shadow-[0_15px_40px_-10px_rgba(225,29,72,0.25)] hover:border-primary-100 dark:hover:border-primary-900/50 transition-all duration-300 flex items-start gap-4 mb-8 animate-slide-up delay-200">
          <div className="p-3 bg-primary-50 dark:bg-primary-950/20 text-primary-600 rounded-2xl flex-shrink-0 transition-colors duration-200">
            <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
          </div>
          <div>
            ...
            <ul className="list-disc pl-4 space-y-1.5 text-sm text-gray-500 dark:text-gray-400 font-medium">
              <li><strong>Comisión de gestión variable:</strong> Minimizas costos a mayor volumen de compra. Comisión dinámica 10% (&lt; $200), 7.5% ($200-$1000) y 5% para presupuestos superiores a $1000.</li>
        ...
        {items.length > 0 && (
          <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-3xl p-6 sm:p-10 shadow-sm hover:shadow-[0_15px_40px_-10px_rgba(225,29,72,0.15)] dark:hover:shadow-[0_15px_40px_-10px_rgba(225,29,72,0.25)] hover:border-primary-100 dark:hover:border-primary-900/50 transition-all duration-300 animate-scale-in">
            ...
            <div className="flex justify-between items-center mb-6">
              ...
              <div className="text-right">
                <p className="text-2xl font-black text-primary-600 dark:text-primary-500">${totals.totalUsd.toFixed(2)}</p>
                {tasaVes && <p className="text-sm font-bold text-gray-700 dark:text-gray-300 mt-0.5">≈ {totals.totalBs.toFixed(2)} Bs</p>}
              </div>
            </div>

            <div className="bg-primary-50/50 dark:bg-primary-950/20 border border-primary-100/50 dark:border-primary-900/40 rounded-2xl p-4 text-center text-sm font-bold text-primary-700 dark:text-primary-400 mb-8 leading-relaxed transition-colors duration-200">
              <i className="bi bi-info-circle-fill mr-1.5" aria-hidden="true"></i> Financiamiento: Inicial 50% <span className="underline">${totals.inicialUsd.toFixed(2)} ({totals.inicialBs.toFixed(2)} Bs)</span> · Saldo contra entrega <span className="underline">${totals.inicialUsd.toFixed(2)} ({totals.inicialBs.toFixed(2)} Bs)</span>
            </div>
        ...
        {showCheckoutModal && (
          ...
          >
            <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-3xl p-6 sm:p-8 max-w-md w-full shadow-2xl relative transition-colors duration-200">
              <button
                type="button"
                onClick={() => setShowCheckoutModal(false)}
                aria-label="Cerrar modal"
                className="absolute top-4 right-4 text-gray-400 hover:text-gray-900 dark:hover:text-white p-2 rounded-full hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary-500"
              >
              ...
              <h3 id="modal-title" className="text-xl font-black text-gray-900 dark:text-white mb-2">Completar Datos</h3>
              <p className="text-sm text-gray-500 dark:text-gray-400 mb-6 font-medium">Por favor ingresa tu nombre y número de contacto para asociar la cotización.</p>
              <form onSubmit={(e) => { e.preventDefault(); processOrder(anonNombre, anonTelefono); }} className="space-y-4" noValidate>
                <div>
                  <label htmlFor="anonNombre" className="block text-xs font-bold uppercase tracking-wider text-gray-500 dark:text-gray-400 mb-2">Nombre completo</label>
                  <input
                    id="anonNombre"
                    type="text"
                    required
                ...
                <div>
                  <label htmlFor="anonTelefono" className="block text-xs font-bold uppercase tracking-wider text-gray-500 dark:text-gray-400 mb-2">WhatsApp</label>
                  <input
                    id="anonTelefono"
                    type="tel"
                    required
